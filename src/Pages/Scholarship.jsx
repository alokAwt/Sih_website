...
export default function Scholarship() {
  ...
  const Schlorship = async () => {
    let res = await fetch(
      `https://sih-backend-ivory.vercel.app/api/v1/Government/AllSchlorship`
    );
    res = await res.json();
    console.log("alok", res.data);
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 p-4">
        {data && data.length > 0
          ? data.map((scholarship, index) => <ScholarshipCard  key={index} {...scholarship}/>)
          : null}
      </div>
    </div>
  );
